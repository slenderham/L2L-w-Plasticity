...
for i in tqdm.tqdm(range(train_epochs), position=0, leave=True):
    ...
    with torch.no_grad():
        while(not done):
            ...
            states.append(torch.cat([new_v[0].squeeze(), mod.squeeze()], dim=-1));
    ...
    for i in tqdm.tqdm(range(val_epochs), position=0, leave=True):
        ...
        with torch.no_grad():
            while(not done):
                ...
                # one iter of network, notice that the reward is from the previous time step
                new_v, new_h, new_dU, new_trace, (last_layer_out, log_probs, value), mod = model.eval().forward(\
                                                            x = total_input.to(device),\
                                                            h = new_h, \
                                                            v = new_v, \
                                                            dU = new_dU, \
                ...
                states[-1].append([new_v[0], new_dU[0], mod[0]]);
...
mods = [[j[-1].squeeze().detach().numpy() for j in t] for t in states]
...
results, dictvec, feats_flat, acts_flat = trajectory.linear_regression_fit(feats, mods);

# h2modweight = model.rnns[0].mod2h.weight[1].flatten().detach().numpy();
# sig_r_coeff_pos = [i for i in range(32) if results[i].pvalues[1]<0.05];
# sig_q_coeff_pos = [i for i in range(32) if results[i].pvalues[3]<0.05];
# r_coeffs = [results[i].params[1]  for i in sig_r_coeff_pos];
# q_coeffs = [results[i].params[3] for i in sig_q_coeff_pos];

# fig, axes = plt.subplots(1, 2);
# axes[0].scatter(h2modweight[sig_r_coeff_pos], r_coeffs);
# axes[0].plot(h2modweight[sig_r_coeff_pos], np.poly1d(np.polyfit(h2modweight[sig_r_coeff_pos], r_coeffs, 1))(h2modweight[sig_r_coeff_pos]));
# print(stats.pearsonr(h2modweight[sig_r_coeff_pos], r_coeffs));
# axes[1].scatter(h2modweight[sig_q_coeff_pos], q_coeffs);
# axes[1].plot(h2modweight[sig_q_coeff_pos], np.poly1d(np.polyfit(h2modweight[sig_q_coeff_pos], q_coeffs, 1))(h2modweight[sig_q_coeff_pos]));
