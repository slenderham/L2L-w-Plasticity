...
for i in tqdm.tqdm(range(train_epochs), position=0, leave=True):
    ...
    loss = 0
    for _ in range(num_rules_per_ep):
        rule_train_now = hp['rng'].choice(hp['rule_trains'], p=hp['rule_probs'])
        sub_trial = tasks.generate_trials(rule_train_now, hp, 'random',
                batch_size=hp['batch_size_train'])

        # support set for quick task adaptation
        sub_inputs = torch.from_numpy(sub_trial.x)
        sub_targets = torch.from_numpy(sub_trial.y)
        sub_c_mask = torch.from_numpy(sub_trial.c_mask)

        new_v, new_h, new_dU, new_trace, (last_layer_out, last_layer_fws, output, value), mod = model.train().forward(\
                                                            x = sub_inputs.to(device),\
                                                            h = new_h, \
                                                            v = new_v, \
                                                            dU = new_dU, \
                                                            trace = new_trace);

        loss += torch.mean(sub_c_mask.to(device)*((output-sub_targets.to(device))**2).flatten(0, 1))/num_rules_per_ep
        
        fixation_mask = (torch.argmax(sub_targets[-1], dim=-1)==0).float();
        angle_diffs = (torch.argmax(output[-1], dim=-1)-torch.argmax(sub_targets[-1].to(device), dim=-1))*2*np.pi/n_eachring
