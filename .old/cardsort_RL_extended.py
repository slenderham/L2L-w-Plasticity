...
def sample_card(chunks, val, dim):
    randInts = torch.argsort(torch.rand(num_cards, chunks), dim=0);
    correctInd = torch.from_numpy(np.random.randint(0, num_cards, size=(1,))).squeeze();
    query = torch.zeros(chunks).long()
    query[dim] = randInts[correctInd, dim];
    for i in range(0, chunks):
        if (i!=dim):
            query[i] = [randInts[c, i] for c in range(num_cards) if c!=correctInd][torch.randint(num_cards-1, (1,)).long()]
    randInts = torch.cat([randInts, query.reshape(1, chunks).long()], dim=0)
    randInts = randInts.reshape((num_cards+1)*(chunks))
    pattern = data[randInts, :].reshape(num_cards+1, chunks, val).to(device)
    pattern = torch.cat([pattern, torch.zeros(num_cards+1, chunks, num_cards+1, device=device)], dim=2) # num_cards+1, chunks, val+num_cards+1
    numbering = torch.cat([torch.eye(num_cards, device=device).reshape(num_cards, 1, num_cards), torch.zeros(1, 1, num_cards, device=device)], dim=0) # num_cards+1, 1, num_cards
    numbering = torch.cat([torch.zeros(num_cards+1, 1, val, device=device), numbering, torch.zeros(num_cards+1, 1, 1, device=device)], dim=-1) # num_cards+1, 1, val+num_cards+1
    pattern = torch.cat((pattern, numbering), dim=1);
    pattern = pattern.reshape((num_cards+1)*(chunks+1), 1, val+num_cards+1)
...
#     randInts = torch.cat([randInts, query.reshape(1, chunks).long()], dim=0)
#     randInts = randInts.reshape((num_cards+1)*(chunks))
#     pattern = data[randInts, :].reshape(num_cards+1, chunks, val).flatten(-2, -1).to(device) # num_cards+1, chunks*(val)
#     pattern = torch.cat([pattern.reshape(num_cards+1, 1, chunks*val), torch.zeros(num_cards+1, 1, num_cards+1, device=device)], dim=2) # num_cards+1, chunks*(val)+num_cards+1
#     return randInts.to(device), pattern.to(device), correctInd.to(device)
...
# number of cards to choose from
num_cards = 4;

sampler = lambda dim: sample_card(chunks, val, dim);

model = SGRU(in_type = "continuous",\
             out_type = "categorical",\
             num_token = 0,\
             input_dim = chunks*val+num_cards+1,\
             hidden_dim = 64,\
             out_dim = num_cards,\
             num_layers = 1,\
             activation="relu",\
...
try:
    state_dict = torch.load("model_WCST_ext");
...
for i in tqdm.tqdm(range(n_epochs), position=0, leave=True):
    ...
    action = torch.zeros(1, num_cards, device=device);
    ...
    with torch.no_grad():
        for idx in range(len_seq):
            instrInts, instrPattern, correctInd = sampler(newDim);
            if (idx==next_change):
                newDim = torch.randint(low=0, high=chunks, size=(1,)).to(device);
                next_change += 20;
            ...
            feedback = torch.cat(
                                    (torch.zeros((1, val*chunks), device=device), action.detach(), reward.detach()), dim=1
                                ).reshape(1, 1, val*chunks+num_cards+1);

            total_input = torch.cat((feedback, instrPattern), dim=0);
            total_input = torch.repeat_interleave(total_input, repeats=3, dim=0)
            ...
            # get reward
            reward = torch.as_tensor((correctInd==action_idx), dtype=torch.float, device=device).reshape(1,-1).detach();
    ...
    if (i+1)%buffer_size==0:
        ...
        torch.save({'model_state_dict': model.state_dict(), \
                    ...
                    'cumReward': cumReward}, 'model_WCST_ext');
...
# axe = vis_lda(hs.flatten(0,1).detach(), dims.flatten());
# axe.set_title("LDA of Cell State")
# axe = vis_lda(dUs.flatten(2,3).flatten(0,1).detach(), dims.flatten());
# axe.set_title("LDA of Fast Weight")
