...
def generator(config):
    ...
    num_train_val_test = [int(config.num_image_per_class*float(ratio)/np.sum(config.train_val_test_ratio)) for ratio in config.train_val_test_ratio];
    ...
    for i, split_name in enumerate(['train', 'val', 'test']):
        ...
        for j, current_class in enumerate(classes):
            ...
            for k in range(num_train_val_test[i]):
                # sample images
